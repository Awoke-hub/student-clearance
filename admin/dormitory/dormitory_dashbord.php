...
// Check if user is logged in and is a dormitory admin
if (!isset($_SESSION['admin_role']) || $_SESSION['admin_role'] !== 'dormitory_admin') {
...
// NEW FUNCTION: Check if department has approved this student (locks dormitory)
function isLockedForDormitory($student_id, $conn) {
    $stmt = $conn->prepare("SELECT status FROM department_clearance WHERE student_id = ? AND status = 'approved'");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['action_type']) && !isset($_POST['bulk_action'])) {
        ...
        if ($action_type === 'approve') {
            ...
            $check_stmt = $conn->prepare("SELECT student_id FROM dormitory_clearance WHERE id = ?");
            ...
            if ($student_data) {
                ...
                if (isLockedForDormitory($student_id, $conn)) {
                    $error_message = "Cannot modify: Department has already approved this student!";
                } else {
                    ...
                    ");
                    $clearance_check->bind_param("ss", $student_id, $student_id);
                    $clearance_check->execute();
                    $clearance_result = $clearance_check->get_result();
                    $clearance_data = $clearance_result->fetch_assoc();
                    
                    $library_status = $clearance_data['library_status'] ?? 'not_requested';
                    $cafeteria_status = $clearance_data['cafeteria_status'] ?? 'not_requested';
                    
                    if ($library_status === 'approved' && $cafeteria_status === 'approved') {
                        // Both clearances are approved, proceed with dormitory approval
                        $stmt = $conn->prepare("UPDATE dormitory_clearance SET status='approved', reject_reason=NULL WHERE id=?");
                        ...
                        $error_message = "Cannot approve: Student must be cleared by both Library and Cafeteria first.";
                    }
                }
            }
            ...
            if ($student_data) {
                ...
                if (isLockedForDormitory($student_id, $conn)) {
                    ...
                    if (!empty(trim($reject_reason))) {
                        $stmt = $conn->prepare("UPDATE dormitory_clearance SET status='rejected', reject_reason=? WHERE id=?");
        ...
        if (!empty($selected_requests)) {
            ...
            if ($bulk_action === 'approve') {
                ...
                if ($processed_count > 0) {
                    $success_message = $processed_count . " request(s) approved successfully!";
                    $additional_messages = [];
                    if ($locked_count > 0) {
                        $additional_messages[] = $locked_count . " request(s) locked (approved by department)";
                    }
                    if ($clearance_failed_count > 0) {
                        $additional_messages[] = $clearance_failed_count . " request(s) need library & cafeteria approval";
                    }
                    ...
                    if ($locked_count > 0) {
                        $error_message = "Selected requests are locked and cannot be modified (approved by department).";
                    } elseif ($clearance_failed_count > 0) {
                        $error_message = "No requests could be approved. All selected requests require both Library and Cafeteria clearance first.";
                    }
                }
            } 
            elseif ($bulk_action === 'reject') {
                ...
                if (!empty(trim($reject_reason))) {
                    ...
                    if ($processed_count > 0) {
                        $success_message = $processed_count . " request(s) rejected successfully!";
                        if ($locked_count > 0) {
                            $error_message = $locked_count . " request(s) could not be processed (approved by department).";
                        }
...
// Get statistics from dormitory_clearance table - ONLY FOR APPROVED BY LIBRARY & CAFETERIA AND CURRENT ACADEMIC YEAR
...
// Get requests for the main table - ONLY STUDENTS APPROVED BY LIBRARY & CAFETERIA AND CURRENT ACADEMIC YEAR
$search = $_GET['search'] ?? '';
$status_filter = $_GET['status'] ?? 'all';

// Query to get dormitory requests with library, cafeteria, and department status - ONLY FOR LIBRARY & CAFETERIA APPROVED STUDENTS AND CURRENT ACADEMIC YEAR
$main_query = "
    SELECT 
        dc.*, 
...
// ORDER BY to prioritize pending requests with both clearances approved
$main_query .= " ORDER BY 
    priority_order DESC,
...
<body>
    <header class="admin-header">
        <h1 style="font-size: 1.4rem;"><i class="fas fa-bed"></i> Dormitory Admin Dashboard</h1>
    ...
    <nav class="admin-nav">
        <h2 style="font-size: 1.2rem; margin: 0;"><i class="fas fa-tachometer-alt"></i> Dormitory Clearance 
    ...
    <div class="main-content">
        ...
        <div class="clearance-notice">
            ...
            <div>
                ...
                • Only showing students already approved by both Library and Cafeteria
                • Look for the <span style="color: var(--success);">⭐ star indicator</span> on ready-to-approve requests
                • Once Department approves, this department gets locked
            </div>
        </div>
        ...
        <div class="section">
            <div class="section-header">
                <h3 class="section-title">
                    ...
                    Dormitory Clearance Requests (<?php echo $current_academic_year; ?> Academic Year)
                </h3>
                        ...
                        <table>
                            ...
                            <tbody>
                                <?php while ($request = $all_requests->fetch_assoc()): 
                                    // Get library, cafeteria, and department status
                                    ...
                                    // Check if locked (department has approved)
                                    $is_locked = ($department_status === 'approved');
                                    
                                    $can_approve = ($library_status === 'approved' && $cafeteria_status === 'approved' && !$is_locked);
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($priority): ?>
                                            <span class="priority-indicator" title="Ready for approval - Both Library and Cafeteria cleared">
                                    ...
                                    <td>
                                        <span class="clearance-badge clearance-<?php echo $department_status; ?>" title="Department: <?php echo ucfirst($department_status); ?>">
                                            <i class="fas fa-<?php 
                                                echo $department_status === 'approved' ? 'check' : 
                                                     ($department_status === 'pending' ? 'clock' : 
                                                     ($department_status === 'rejected' ? 'times' : 'question')); 
                                            ?>"></i>
                                            <?php echo ucfirst($department_status); ?>
                                        </span>
                                    </td>
                                    <td>
                                        <?php if ($is_locked): ?>
            ...
            <?php else: ?>
                <div class="empty-state">
                    ...
                    <p>There are currently no <?php echo $current_academic_year; ?> clearance requests from students who have been approved by both Library and Cafeteria.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <script>
        ...
        function handleActionChange(requestId, currentStatus, canApprove, showApprove, showReject) {
            ...
            if (selectedAction === 'approve') {
                if (canApprove) {
                    ...
                    alert('Cannot approve: Student must be cleared by both Library and Cafeteria first. Please check the clearance status columns.');
                    dropdown.value = '';
                }
        ...
        function applyBulkAction() {
            ...
            if (action === 'approve') {
                if (confirm(`Are you sure you want to approve ${selectedCount} request(s)? Only requests with both Library and Cafeteria approval will be processed.`)) {

...
// Check if user is logged in and is a cafeteria admin
if (!isset($_SESSION['admin_role']) || $_SESSION['admin_role'] !== 'cafeteria_admin') {
...
// CORRECTED FUNCTION: Check if dormitory has approved this student (locks cafeteria)
function isLockedForCafeteria($student_id, $conn, $academic_year) {
    $stmt = $conn->prepare("SELECT status FROM dormitory_clearance WHERE student_id = ? AND academic_year = ? AND status = 'approved'");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['action_type']) && !isset($_POST['bulk_action'])) {
        ...
        if ($action_type === 'approve') {
            ...
            $check_stmt = $conn->prepare("SELECT student_id FROM cafeteria_clearance WHERE id = ? AND academic_year = ?");
            $check_stmt->bind_param("ii", $request_id, $currentYear);
            $check_stmt->execute();
            ...
            if ($student_result->num_rows > 0) {
                ...
                // Check if locked (dormitory has approved) - PASS currentYear
                if (isLockedForCafeteria($student_id, $conn, $currentYear)) {
                    $error_message = "Cannot modify: Dormitory has already approved this student for $currentYear!";
                } else {
                    // Check if student is approved by library - ADD year filter
                    $library_check = $conn->prepare("
                        SELECT status FROM library_clearance 
                        WHERE student_id = ? AND academic_year = ?
                        ORDER BY id DESC 
                        LIMIT 1
                    ");
                    $library_check->bind_param("si", $student_id, $currentYear);
                    $library_check->execute();
                    $library_result = $library_check->get_result();
                    $library_data = $library_result->fetch_assoc();
                    
                    $library_status = $library_data['status'] ?? 'not_requested';
                    
                    if ($library_status === 'approved') {
                        // Library has approved, proceed with cafeteria approval
                        $stmt = $conn->prepare("UPDATE cafeteria_clearance SET status='approved', reject_reason=NULL WHERE id=? AND academic_year = ?");
                        $stmt->bind_param("ii", $request_id, $currentYear);
                        if ($stmt->execute()) {
                            $success_message = "Student approved successfully for $currentYear!";
                        } else {
                            $error_message = "Error updating approval status.";
                        }
                    } else {
                        $error_message = "Cannot approve: Student must be cleared by Library first for $currentYear.";
                    }
                }
            } else {
            ...
            if ($student_result->num_rows > 0) {
                ...
                if (isLockedForCafeteria($student_id, $conn, $currentYear)) {
                    ...
                    if (!empty(trim($reject_reason))) {
                        $stmt = $conn->prepare("UPDATE cafeteria_clearance SET status='rejected', reject_reason=? WHERE id=? AND academic_year = ?");
        ...
        if (!empty($selected_requests)) {
            ...
            if ($bulk_action === 'approve') {
                ...
                if ($processed_count > 0) {
                    ...
                    $additional_messages = [];
                    if ($locked_count > 0) {
                        $additional_messages[] = $locked_count . " request(s) locked (approved by dormitory)";
                    }
                    if ($library_failed_count > 0) {
                        $additional_messages[] = $library_failed_count . " request(s) need library approval";
                    }
                    if ($error_count > 0) {
                        $additional_messages[] = $error_count . " request(s) had errors";
                    }
                    if (!empty($additional_messages)) {
                        $success_message .= " - " . implode(", ", $additional_messages);
                    }
                } else {
                    if ($locked_count > 0) {
                        $error_message = "Selected requests are locked and cannot be modified (approved by dormitory).";
                    } elseif ($library_failed_count > 0) {
                        $error_message = "No requests could be approved. All selected requests require Library clearance first for $currentYear.";
                    } else {
            ...
            elseif ($bulk_action === 'reject') {
                ...
                if (!empty(trim($reject_reason))) {
                    ...
                    if ($processed_count > 0) {
                        ...
                        if ($locked_count > 0) {
                            $error_message = $locked_count . " request(s) could not be processed (approved by dormitory).";
                        }
                        if ($error_count > 0) {
...
// Get statistics from cafeteria_clearance table - MODIFIED to include year filter
...
");
$stats_stmt->bind_param("ii", $currentYear, $currentYear);
$stats_stmt->execute();
...
// Get requests for the main table - MODIFIED queries to include year filter
$search = $_GET['search'] ?? '';
$status_filter = $_GET['status'] ?? 'all';

// Query to get cafeteria requests with library and dormitory status - MODIFIED with year filters
$main_query = "
    SELECT 
        cc.*, 
...
if (!empty($search)) {
    $search_term = "%$search%";
    $main_query .= " AND (cc.name LIKE ? OR cc.student_id LIKE ? OR cc.last_name LIKE ?)";
}

if ($status_filter !== 'all') {
    $main_query .= " AND cc.status = ?";
}

// ORDER BY to prioritize pending requests with library approved
$main_query .= " ORDER BY 
    priority_order DESC,
    cc.status = 'pending' DESC,
    cc.requested_at DESC";

$main_stmt = $conn->prepare($main_query);

// Dynamic parameter binding - MODIFIED to include currentYear parameters
$param_types = 'iiiii'; // 5 parameters for currentYear
$param_values = [$currentYear, $currentYear, $currentYear, $currentYear, $currentYear];
...
<head>
    ...
    <title>Cafeteria Admin Dashboard</title>
    ...
    <style>
        ...
        * {
        ...
        /* MINIMAL ADDITIONS FOR LOCKED STATE */
        .status-locked { 
            background: linear-gradient(135deg, #e9ecef, #dee2e6);
            color: #6c757d;
            border: 1px solid #ced4da;
...
<body>
    <header class="admin-header">
        <h1 style="font-size: 1.4rem;"><i class="fas fa-utensils"></i> Cafeteria Admin Dashboard</h1>
        <div style="display: flex; align-items: center; gap: 1rem; font-size: 0.9rem;">
            <span><i class="fas fa-user-circle"></i> <?php echo htmlspecialchars($_SESSION['admin_name'] . ' ' . $_SESSION['admin_last_name']); ?></span>
    ...
    <nav class="admin-nav">
        <h2 style="font-size: 1.2rem; margin: 0;"><i class="fas fa-tachometer-alt"></i> Clearance Management</h2>
        <div class="filters">
    ...
    <div class="main-content">
        ...
        <div class="stats-container">
            <div class="stat-card total">
                ...
                <div class="stat-label">Total Requests (<?php echo $currentYear; ?>)</div>
            </div>
            <div class="stat-card pending">
                <div class="stat-icon">⏳</div>
                <div class="stat-number"><?php echo $stats['pending'] ?? 0; ?></div>
                <div class="stat-label">Pending (<?php echo $currentYear; ?>)</div>
            </div>
            <div class="stat-card approved">
                <div class="stat-icon">✅</div>
                <div class="stat-number"><?php echo $stats['approved'] ?? 0; ?></div>
                <div class="stat-label">Approved (<?php echo $currentYear; ?>)</div>
            </div>
            <div class="stat-card rejected">
                <div class="stat-icon">❌</div>
                <div class="stat-number"><?php echo $stats['rejected'] ?? 0; ?></div>
                <div class="stat-label">Rejected (<?php echo $currentYear; ?>)</div>
            </div>
        </div>
        ...
        <div class="auto-approve-notice">
            ...
            <div>
                <strong>Clearance Rules for <?php echo $currentYear; ?>:</strong> 
                • Only showing students already approved by Library
                • Look for the <span style="color: var(--success);">⭐ star indicator</span> on ready-to-approve requests
                • Once Dormitory approves, this department gets locked
            </div>
        </div>
        ...
        <div class="section">
            <div class="section-header">
                <h3 class="section-title">
                    ...
                    Clearance Requests (<?php echo $currentYear; ?> - Library Approved Students Only)
                </h3>
                        ...
                        <table>
                            ...
                            <tbody>
                                <?php while ($request = $all_requests->fetch_assoc()): 
                                    // Get library and dormitory status
                                    $library_status = $request['library_status'] ?? 'not_requested';
                                    $dormitory_status = $request['dormitory_status'] ?? 'not_requested';
                                    
                                    // Check if locked (dormitory has approved) - PASS currentYear
                                    $is_locked = isLockedForCafeteria($request['student_id'], $conn, $currentYear);
                                    
                                    $can_approve = ($library_status === 'approved' && !$is_locked);
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($priority): ?>
                                            <span class="priority-indicator" title="Ready for approval - Library cleared">
                                    ...
                                    <td>
                                        <span class="clearance-badge clearance-<?php echo $library_status; ?>" title="Library: <?php echo ucfirst($library_status); ?> (<?php echo $currentYear; ?>)">
                                            <i class="fas fa-<?php 
                                    ...
                                    <td>
                                        <span class="clearance-badge clearance-<?php echo $dormitory_status; ?>" title="Dormitory: <?php echo ucfirst($dormitory_status); ?> (<?php echo $currentYear; ?>)">
                                            <i class="fas fa-<?php 
                                        ...
                                        <?php if ($is_locked): ?>
                                            ...
                                            <?php if ($request['status'] === 'approved'): ?>
                                                <div class="locked-info">
                                                    <i class="fas fa-info-circle"></i>
                                                    Previously Approved (<?php echo $currentYear; ?>)
                                                </div>
                                            <?php elseif ($request['status'] === 'rejected'): ?>
                                                <div class="locked-info">
                                                    <i class="fas fa-info-circle"></i>
                                                    Previously Rejected (<?php echo $currentYear; ?>)
                                                </div>
                                            <?php endif; ?>
                                        <?php else: ?>
                                    ...
                                    <td class="actions">
                                        ...
                                        <?php else: ?>
                                            <span style="color: #6c757d; font-size: 0.85rem; display: flex; align-items: center; gap: 0.4rem;">
            ...
            <?php else: ?>
                <div class="empty-state">
                    ...
                    <p>There are currently no clearance requests from students who have been approved by Library for the <?php echo $currentYear; ?>-<?php echo $currentYear + 1; ?> academic year.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <script>
        ...
        function handleActionChange(requestId, currentStatus, canApprove, showApprove, showReject) {
            ...
            if (selectedAction === 'approve') {
                if (canApprove) {
                    ...
                    alert('Cannot approve: Student must be cleared by Library first. Please check the Library Status column.');
                    dropdown.value = '';
                }
        ...
        function applyBulkAction() {
            ...
            if (action === 'approve') {
                if (confirm(`Are you sure you want to approve ${selectedCount} request(s)? Only requests with Library approval will be processed.`)) {
